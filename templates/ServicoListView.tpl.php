<?php
	$this->assign('title','SISTEMA_IGCAR | Servicos');
	...
	<script type="text/template" id="servicoCollectionTemplate">
		...
		<thead>
			<tr>
				...
				<th id="header_Servico">Servico<% if (page.orderBy == 'Servico') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
		...
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('id')) %>">
				...
				<td><%= _.escape(item.get('servico') || '') %></td>
	...
	<script type="text/template" id="servicoModelTemplate">
		<form class="form-horizontal" onsubmit="return false;">
			<fieldset>
				...
				<div id="servicoInputContainer" class="control-group">
					<label class="control-label" for="servico">Servico</label>
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="servico" placeholder="Servico" value="<%= _.escape(item.get('servico') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
